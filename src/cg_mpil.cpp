...
double inner_product(std::vector<double>& a,
                     std::vector<double>& b,
                     double* local_sum_ptr,
                     double* global_sum_ptr,
                     MPIL_Comm* mpil_comm,
                     MPIL_Request* mpil_req)
{
    *local_sum_ptr = 0;
    ...
    if (mpil_req == NULL)
    {
        MPIL_Allreduce(local_sum_ptr, global_sum_ptr, 1, MPI_DOUBLE, MPI_SUM, mpil_comm);
    }
    else
    {
        MPIL_Start(mpil_req);
       ...
       bool allreduce_init)
{
    ...
    rr_inner = inner_product(r, r, &local_sum, &global_sum, mpil_comm, mpil_req);
    ...
    while (norm_r > tol && iter < max_iter)
    {
        ...
        App_inner = inner_product(Ap, p, &local_sum, &global_sum, mpil_comm, mpil_req);
        if (App_inner < 0.0)
        {
        ...
        next_inner = inner_product(r, r, &local_sum, &global_sum, mpil_comm, mpil_req);
...
int main(int argc, char* argv[])
{
    ...
    for (const auto& neighbor_config : neighbor_configs)
    {
        ...
        {
            MPIL_Set_alltoallv_neighbor_init_algorithm(neighbor_config.init_method);
        }
        else
        {
            MPIL_Set_alltoallv_neighbor_algorithm(neighbor_config.method);
        }
